...
func main(){  
  ...
  for i:=range(KitchenList){
    KitchenList[i].BuildPlaceDistanceList()  
  }  
  ...
  for i:=range(PlaceList){
    PlaceList[i].BuildKitchenDistanceList()    
    PlaceList[i].BuildPlaceDistanceList()
  }
  
  //clustering!
  GreedyClusteringForPlace()  
  for i:=range(KitchenList){
    fmt.Println(KitchenList[i].Index,KitchenList[i].Capacity.Min,KitchenList[i].Capacity.Max,KitchenList[i].DistinctOrderCount,KitchenList[i].OrderQty)
  }  
  
  //do the maximum_matching  
  GreedyClusterOptimizationForPlace(OPTIMIZATION_REPETITION) 
  fmt.Println("-----------------------------")
  for i:=range(KitchenList){
