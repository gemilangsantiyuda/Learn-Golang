...
func (kitchen *Kitchen) CanServePlace(place *Place) bool{
  kitchenHasEnoughCapacity:=(kitchen.OrderQty + place.Qty <= kitchen.Capacity.Max)
  isCloseEnough:= (GetHaversineDistance(kitchen.Coord,place.Coord)<=MAX_PATH_LENGTH)
  return kitchenHasEnoughCapacity && isCloseEnough
}

func (kitchenOrigin *Kitchen) GivePlaceToKitchen(place *Place, kitchenDestination *Kitchen){
  kitchenDestination.OrderQty += place.Qty
  kitchenDestination.DistinctOrderCount+= len(place.OrderList)
  kitchenOrigin.DistinctOrderCount-= len(place.OrderList)
  kitchenOrigin.OrderQty -= place.Qty
  
  place.ServingKitchen =kitchenDestination
  for i:= range place.OrderList{
    order:= place.OrderList[i]
    order.ServingKitchen= kitchenDestination
  }  
}

func (kitchen *Kitchen) CanLetGoPlace(place *Place) bool{
  kitchenHasEnoughOrder:=(kitchen.OrderQty-place.Qty>=kitchen.Capacity.Min)
  return kitchenHasEnoughOrder 
}
